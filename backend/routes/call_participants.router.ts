...
import {
    ...
    getCallParticipantsSchema
} from '../schema/call_participants.schema.js';
...
router.get('/call/:callId',
    validatorHandler(getCallParticipantsSchema, 'params'),
    async (req: Request, res: Response) => {
        try {
            const { callId } = req.params;
            const participants = await service.findByCall(callId as string);
            res.json(participants);
        } catch (error: any) {
