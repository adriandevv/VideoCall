...
router.get('/:id',
    validatorHandler(getCallSchema, 'params'),
    async (req: Request, res: Response) => {
        try {
            const { id } = req.params;
            const call = await service.findOne(id as string);
            res.json(call);
        } catch (error: any) {
...
router.post('/',
    validatorHandler(createCallSchema, 'body'),
    async (req: Request, res: Response) => {
        try {
            const body = req.body;
...
router.patch('/:id',
    validatorHandler(getCallSchema, 'params'),
    validatorHandler(updateCallSchema, 'body'),
    async (req: Request, res: Response) => {
        try {
            const { id } = req.params;
            const body = req.body;
            const result = await service.update(id as string, body);
            res.json(result);
        } catch (error: any) {
